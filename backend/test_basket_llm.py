"""
Test script for the basket-llm-no-target API endpoint.
Creates a basket using OpenAI LLM to identify themes and assign weights based on thematic centrality.
...
# API configuration
API_URL = "http://localhost:8000/api/basket-llm-no-target"
...
def test_basket_llm():
    """Call the basket-llm-no-target API and display results."""
    
    print("=" * 80)
    print("TESTING BASKET-LLM-NO-TARGET API")
    print("=" * 80)
    ...
    print("⏳ This may take 10-30 seconds (waiting for OpenAI response)...")
    ...
    try:
        ...
        print("\n" + "-" * 80)
        print("LLM-ASSIGNED WEIGHTS (with reasoning)")
        print("-" * 80)
        
        # Weights are already sorted by the LLM
        ...
        for i, w in enumerate(weights_list, 1):
            ...
            reasoning = w.get('reasoning', 'No reasoning provided')
        ...
        total_weight = sum(w['weight'] for w in weights_list)
        print("\n" + "-" * 80)
        ...
        if TARGET_MARKET_ID:
            ...
            try:
                ...
                input_series_list = []
                basket_market_ids = [w['market_id'] for w in weights_list]
                basket_weights = {w['market_id']: w['weight'] for w in weights_list}
                ...
                if df.empty:
                    ...
                    plt.plot(timestamps, synthetic_prices, label='Synthetic Basket (LLM Method)', color='green', linewidth=2, linestyle='--', marker='x', markersize=3)
                    plt.title(f'LLM Basket vs Target Market (R² = {r_squared:.4f})\nTarget: {target_question}\nTheme: {result["theme"]}', fontsize=12, fontweight='bold')
